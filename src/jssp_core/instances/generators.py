...
from jssp_core.instances import get_instance, get_transport_instance
from jssp_core.instances.agv import (
    JSSPTransportInstance,
    generate_grid_random_transport_instance,
)
...
class InstanceGenerator(Protocol):
    ...
    def generate(self, seed: int | None = None) -> JSSPInstance | JSSPTransportInstance:
    ...
    def __next__(self) -> JSSPInstance | JSSPTransportInstance:
        """Yield the next instance so the generator is iterable."""
        ...


InstanceGeneratorLike = (
    InstanceGenerator | Callable[..., JSSPInstance | JSSPTransportInstance]
)
...
class PredefinedInstanceGenerator:
    ...
    def __init__(
        self,
        instances: list[JSSPInstance | JSSPTransportInstance | str],
    ...
    def generate(self, seed: int | None = None) -> JSSPInstance | JSSPTransportInstance:
        ...
        if isinstance(instance, (JSSPInstance, JSSPTransportInstance)):
            return instance
...
@INSTANCE_GENERATOR_REGISTRY.register(
    "transport_random_uniform", "transport_uniform_random", "transport_default"
)
@dataclass
class RandomTransportInstanceGenerator:
    """
    Default generator that replicates the previous uniform random distribution.
    """
    ...
    def generate(self, seed: int | None = None) -> JSSPTransportInstance:
        return get_transport_instance(
            {
                "type": "random",
                "num_jobs": self.num_jobs,
                "num_machines": self.num_machines,
    ...
    def __iter__(self) -> RandomTransportInstanceGenerator:
        return self

    def __next__(self) -> JSSPTransportInstance:
        return self.generate()


@INSTANCE_GENERATOR_REGISTRY.register("grid_uniform")
@dataclass
class GridRandomTransportInstanceGenerator:
    """
    Default generator that replicates the previous uniform random distribution.
    """
    ...
    min_travel_duration: int = 1
    max_travel_duration: int = 100

    def generate(self, seed: int | None = None) -> JSSPTransportInstance:
        return generate_grid_random_transport_instance(
            num_jobs=self.num_jobs,
            num_machines=self.num_machines,
            min_op_duration=self.min_op_duration,
            max_op_duration=self.max_op_duration,
            min_travel_duration=self.min_travel_duration,
...
@INSTANCE_GENERATOR_REGISTRY.register("grid")
@dataclass
class GridTransportInstanceGenerator:
    """
    Default generator that replicates the previous uniform random distribution.
    """
        ...
        return get_transport_instance(
            {
                ...
                "min_op_duration": self.min_op_duration,
                "max_op_duration": self.max_op_duration,
                "min_transport_duration": self.min_transport_duration,
                "max_transport_duration": self.max_transport_duration,
                "seed": seed,
            }
        )
...
@INSTANCE_GENERATOR_REGISTRY.register("transport_truncated_normal")
@dataclass
class TruncatedNormalTransportInstanceGenerator:
    """
    Default generator that replicates the previous uniform random distribution.
    """
    ...
    def __iter__(self) -> TruncatedNormalTransportInstanceGenerator:
        return self
...
@INSTANCE_GENERATOR_REGISTRY.register("norm_truncated_normal")
@dataclass
class NormTruncatedNormalTransportInstanceGenerator:
    """
    Default generator that replicates the previous uniform random distribution.
    """
    ...
    def __iter__(self) -> NormTruncatedNormalTransportInstanceGenerator:
        return self
...
__all__ = [
    ...
    "NormTruncatedNormalTransportInstanceGenerator",
    "PredefinedInstanceGenerator",
    "GridTransportInstanceGenerator",
...
if __name__ == "__main__":
    gen = initialize_instance_generator(
        "truncated_normal",  # transport_random_uniform transport_truncated_normal
        num_jobs=4,
        num_machines=6,
    )
    print(gen)
