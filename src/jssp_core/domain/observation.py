...
class ObservationData(TypedDict, total=False):
    ...
    Keys:
        ...
        # AGV-specific
        available_time_for_agvs (np.ndarray): When each AGV becomes available.
        agv_utlity_rate (np.ndarray): Current utilization rate of AGVs.
        # ... other AGV and variant specific keys ...
    """
    ...
    # For AGV-specific observations
    possible_arrival_time_for_agvs_2_all_locations: np.ndarray
    available_time_for_agvs: np.ndarray
    selected_machine_from_job_selector: np.ndarray
    possible_arrival_time_for_agvs_2_sel_location: np.ndarray
    agv_utlity_rate: np.ndarray
    dummy: np.ndarray
...
class ObservationProvider(ABC):
    ...
    def __init__(self, schedule: Schedule):
        ...
        if isinstance(schedule, TransportSchedule):
            self.num_agvs = schedule.num_agvs
            self.num_machines_wo_loc = schedule.num_machines - 2
        else:
            self.num_agvs = 0
