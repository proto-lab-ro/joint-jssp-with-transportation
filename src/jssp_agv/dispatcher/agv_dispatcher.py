...
from jssp_agv.dispatcher.utils_agv import (
    create_environment,
    # create_gnn_critic,
    create_shared_extractor,
)
from jssp_agv.dispatcher.utils_gnn import (
    create_agv_mlp_policy,
    create_gnn_critic,
...
class AgvDispatcher(MLPDispatcher):
    """Dispatcher for MLP-based training with standard setup."""

    def __init__(self, cfg: DictConfig, job_selector: nn.Module | Heuristic):
    ...
    def setup_environment(self):
        ...
        self.env = create_environment(self.cfg, self.instance, self.job_selector)
        return self.env
    ...
    def setup_test_environment(self):
        ...
        self.test_env = create_environment(
            test_env_cfg, self.instance, self.job_selector
        )
        return self.test_env

    def setup_models(self):
        """Create GNN policies and critic with shared feature extractor."""
        self.policies, self.combined_policies = create_agv_mlp_policy(
            self.env, self.cfg
        )
        shared_extractor = create_shared_extractor(self.env, self.cfg)

        self.value_module = create_gnn_critic(self.env, shared_extractor, self.cfg)
