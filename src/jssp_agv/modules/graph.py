...
def flatten_nested(objs):
    """
    Recursively flattens nested iterables like lists or LinkedLists,
    stopping at torch_geometric.data.Data or torch.Tensor objects.
    """
    flat = []
    for item in objs:
...
class SharedGraphFeatureExtractor(torch.nn.Module):
    ...
    def forward(self, observation):
        ...
        if isinstance(observation, Data):
            ...
            graph_data_list = flatten_nested(observation)
...
class SB3LikeJobActor(torch.nn.Module):
    """
...
class SB3LikeCritic(torch.nn.Module):
    ...
    def __init__(self, shared_extractor, agv_in_dim):
        ...
        self.agent_type = EnvironmentType.MULTI_AGENT
        self.agv_value_head = CriticAgvValueHead(agv_in_dim)
        self.value_head = CriticValueHead(input_dim)

    def forward(self, job_graph_observation, agv_observation):
        """
        Args:
            observation: Dict with 'graph_data' containing PyG Data object
        ...
        """
        _, graph_embedding, _ = self.shared_extractor(job_graph_observation)

        job_state_value = self.value_head(graph_embedding)
        agv_state_value = self.agv_value_head(agv_observation)
        ...
        state_value = job_state_value + agv_state_value
        return state_value
